...
export default function OrderViewerToolbar({ order, onOrderChange, ...props }) {
  ...
  const handleStatusChange = (event) => {
    ...
    if (newStatus === 'processing' && !order.manufacturerId) {
      setAlertOpen(true);
      setStatusError({ title: 'Unassigned Order', message: 'You must select a manufacturer using the “Assign to Manufacturer” tool.' });
      return;
    }
    ...
    if (newStatus ==='shipped') {
      ...
      order.items.forEach(item => {
        if (item.trackingAvailable && (!item.tracking || item.tracking === '')) {
      ...
      if (itemsWithoutTrackingInfo.length > 0) {
        setAlertOpen(true);
        setStatusError({ title: 'Required Data Missing', message: 'Please provide a tracking number or check the box that indicates a tracking number is not available.' });
        return;
      }

      if (itemsNotShipped.length > 0) {
        setAlertOpen(true);
        setStatusError({ title: 'Items not shipped', message: 'Please ensure that all items are flagged as shipped' });
        return;
      }
    }
    ...
    switch (newStatus) {
      case 'pending':        
        setStatusConfirmMessage({ title: 'Going Backward?', message: 'Are you sure you want to take this order back to the Pending Processing status?' });
        break;
      case 'processing':
        setStatusConfirmMessage({ title: 'Send to Manufacturer?', message: 'Are you sure you want send this order to the Manufacturer?' });
        break; 
      case 'shipped':
        setStatusConfirmMessage({ title: 'Mark as Shipped?', message: 'Are you sure you want to mark this order as shipped?' });
        break;
      case 'hold':
        ...
        <Toolbar >
          <Box className={classes.toolbarMainBox}>
            ...
            {!isMfg && (
              ...
              >
                <MenuItem value="in_progress" disabled={true}>In Progress</MenuItem>
