...
export default function OrderList({
  ...
  React.useEffect(() => {
    ...
    if (Auth.currentUserRole === roles.manufacturer) {
      getOrders = API.Orders.getOrdersByManufacturer(
        Auth.currentUserValue.user.manufacturer
      );
    } else {
  ...
  let actions = [
    {
      ...
      onClick: (event, rowData) =>
        history.push(`/orders/${makeGLID(rowData.glid)}`),
    },
  ];
  ...
  return (
    orders && (
      <ListTable
        ...
        options={
          readOnly
            ? { paging: false, search: false, tableLayout: "fixed" }
            : null
        }
        ...
        columns={
          hideCustomer
            ? columns.filter((f) => f.table !== "customers")
            : columns
        }
        data={orders.map((order, index) => {
          ...
          if (Auth.currentUserRole !== roles.manufacturer) {
            ...
            data.itemsShipped = order.items.filter(
              (item) => item.shipped
            ).length;
