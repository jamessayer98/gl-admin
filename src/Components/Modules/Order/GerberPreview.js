  ...
  return (
    <Grid item className={classes.imageCell} {...props}>
      <Paper variant="outlined" className={classes.imageCellPaper}>
        <Box><img alt={label} src={src} className={classes.imageCellImage} /></Box>
        <Typography paragraph variant="caption" className={classes.imageCellCaption}>{label}</Typography>
      </Paper>
    </Grid>
  );
}
...
export default function GerberPreview({ board }) {
  ...
  return (
    <Grid container spacing={2}>
      <Grid item xs={12}>
        <Typography variant="h4" paragraph>{currentImageTitle}</Typography>
        <Paper variant="outlined" className={classes.mainImagePaper}>
          <img alt={currentImageTitle} src={currentImage} className={classes.mainImage}/>
        </Paper>
      </Grid>
      <Grid item xs={12}>
        <Grid container spacing={1}>
          <ImageCell xs={3} key="top" src={board.topImageUrl} label="Top" onClick={() => {
            setCurrentImage(board.topImageUrl);
            setCurrentImageTitle('Top');
            
          }}/>
          <ImageCell xs={3} key="bottom" src={board.bottomImageUrl} label="Bottom" onClick={() => {
            setCurrentImage(board.bottomImageUrl);
            setCurrentImageTitle('Bottom');            
          }}/>
          {board.layers.map((layer, index) => (
            <ImageCell xs={3} key={index} src={layer.imageUrl} label={_getLayerName(layer)} onClick={() => {
