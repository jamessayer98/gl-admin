...
export default function UserList({ history }) {
  ...
  return (
    <React.Fragment>
      <ListTable
        ...
        actions={[
          ...
          rowData => ({
            ...
            disabled: rowData.type === roles.superadmin || authUser === null || authUser.role > rowData.type
          }),
          rowData => ({
        ...
        columns={[
          ...
          { title: 'Enabled', field: 'enabled', render: rowData => rowData.enabled ? <CheckIcon /> : <CloseIcon /> },
      ...
      >
        {selectedRowData && (
          <Typography>Are you sure you want to delete user: <strong>{selectedRowData.name}</strong>?</Typography>
        )}
      </Confirm>
    </React.Fragment>
  );
