...
const defaultEntry = {
  limit: 0.00,
  price: [
      { layers: 2, multiplier: 0.00, offset: 0.00 },
      { layers: 4, multiplier: 0.00, offset: 0.00 }
  ]
};
...
const schema = yup.object().shape({
  categories: yup.array().of(yup.object().shape({
    ...
    price: yup.array().of(yup.object().shape({
      layers: yup.number().required().positive(),
      multiplier: yup.number().required('Multiplier is required').min(0, 'Multiplier must greater than or equal to zero'),
      offset: yup.number().required('Offset is required').min(0, 'Multiplier must greater than or equal to zero'),
    })
  )}))
});
...
export default function PricingTable() {
  ...
  const renderCategoryErrors = (errors) => {
    let messages = [];
    let makeError = (message) => <Typography key={`error-${messages.length + 1}`} variant="body2" color="error">{message}</Typography>
  ...
  const renderCategory = (errors, category, categoryIndex, { push, remove }) => (
        ...
        >
          <TableBody>
            <TableRow key="price-outer">
              <TableCell component="th" scope="row" rowSpan={1 + category.price.length}>
                <Box className={classes.categoryHeaderBox}>
                  <Typography variant="h6" noWrap>Category {String.fromCharCode(categoryIndex + 65)}</Typography>
                  <Box>
                    <IconButton size="small" color="secondary" onClick={() => remove(categoryIndex)}>
            ...
            <formik.FieldArray name={`categories[${categoryIndex}].price`}>
              {(arrayHelpers) => {                                    
                return category.price.map((price, priceIndex) => renderPrices(categoryIndex, price, priceIndex, arrayHelpers))
              }}
            </formik.FieldArray>
          ...
          <formik.Form>
            <Grid container className={classes.outerGrid}>
              <Grid item xs={12} sm={10}>
                <Box p={3}>
                  <Typography variant="h4" paragraph>Pricing Categories</Typography>
                  <formik.FieldArray name="categories" render={(arrayHelpers) => (
                    <>
                      <Grid container spacing={3} className={classes.innerGrid}> 
                        {values.categories.map((category, categoryIndex) => renderCategory(errors, category, categoryIndex, arrayHelpers))}
                      </Grid>
